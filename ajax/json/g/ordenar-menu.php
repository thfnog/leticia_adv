...
if($s->adm()){
	...
	/*if($idc&&$b->query("select * from cat where id=$idc limit 1")->fetchObject())$w[] = "c.idc=$idc";
	...
	if($q)$w[] = "(c.n like '$qel'".($_rel?" or (match(c.n)against('$q' in boolean mode))":'').' )';
	...
	$sql .= ' left join fabricante f on c.idf=c.id';*/

	$sql_total = 'select c.*'.($q&&$_rel?",(match(c.n)against('$q' in boolean mode)) rel":'')." from $sql $w".($_ord?" order by $_ord":'');
