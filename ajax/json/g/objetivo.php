...
if($s->adm()){
	...
	/*if($ids&&$b->query("select * from scat where id=$ids limit 1")->fetchObject())$w[] = "o.ids=$ids";
	if($idf&&$b->query("select * from fabricante where id=$idf limit 1")->fetchObject())$w[] = "o.idf=$idf";*/

	if($q)$w[] = "(o.n like '$qel'".($_rel?" or (match(o.n)against('$q' in boolean mode))":'').' )';
	$w = count($w)?'where '.implode(' and ',$w):'';
	$pgn = pgnl($x->sql_count="$sql $w",$_qt,$_pg);
	if($pgn->e)$pg = $pgn->c;

	/*$sql .= ' left join cat c on o.idc=c.id';
	$sql .= ' left join scat s on o.ids=s.id';
	$sql .= ' left join fabricante f on o.idf=f.id';*/

	$sql = 'select o.*'.
	($q&&$_rel?",(match(o.n)against('$q' in boolean mode)) rel":'').
	" from $sql $w".
	...
	if($st=$b->query($sql)){
		...
		while($rs=$st->fetchObject()){
			...
			$rs->t .= '';
			$rs->q += 0;
			$rf = $b->query("select * from fotos where idp={$rs->id} and tipo='objetivo'")->fetchObject();
			$rpr = $b->query("select * from fotos where idp={$rs->id} and tipo='objetivo' and principal")->fetchObject();
			$img = 'thumb/'.($rpr->iti?$rpr->iti:($rf->iti?$rf->iti:'default.jpg'));
			$rs->i = $img&&file_exists("upload/objetivos/$img")?$img:'';
