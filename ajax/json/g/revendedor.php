...
if($s->adm()||$s->ven()||$s->ger()){
	...
	$sql = 'revenda r';
	$w = array();
	if($q)$w[] = "(r.id='$qe' or r.n like '$qel' or r.cpf like '$qel' or r.cnpj like '$qel'";
	...
	$sql = "select r.* from $sql $w group by r.id".($_ord?" order by $_ord":'')." limit {$pgn->i},{$pgn->l}";
	...
	if($st=$b->query($x->sql=$sql)){
		...
		$x->qel = $qel;
		$x->r = array();
		$tip = array('','Pessoa Física','Pessoa Jurídica');
		while($rs=$st->fetchObject()){
			...
			$rs->email .= '';
			$rs->u .= '';
			$img = $rs->i?$rs->i:'default.jpg';
			$rs->i = $img&&file_exists("upload/revendas/$img")?$img:'';
			$rs->pg = 'revenda';
			$x->r[$i++] = $rs;
		}
		$x->i = $i;
