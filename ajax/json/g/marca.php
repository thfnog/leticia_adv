...
if($s->adm()){
	...
	/*if($ids&&$b->query("select * from scat where id=$ids limit 1")->fetchObject())$w[] = "m.ids=$ids";
	if($idf&&$b->query("select * from fabricante where id=$idf limit 1")->fetchObject())$w[] = "m.idf=$idf";*/

	if($q)$w[] = "(m.n like '$qel' or m.h1 like '$qel'".($_rel?" or (match(m.n)against('$q' in boolean mode))":'').' )';
	$w = count($w)?'where '.implode(' and ',$w):'';
	$pgn = pgnl($x->sql_count="$sql $w",$_qt,$_pg);
	if($pgn->e)$pg = $pgn->c;

	/*$sql .= ' left join cat c on m.idc=m.id';
	$sql .= ' left join scat s on m.ids=s.id';
	$sql .= ' left join fabricante f on m.idf=f.id';*/

	$sql = 'select m.*'.
	($q&&$_rel?",(match(m.n)against('$q' in boolean mode)) rel":'').
	" from $sql $w".
