...
if($s->adm()){
	...
	if($q)$w[] = "(t.nome like '$qel'".($_rel?" or (match(t.nome)against('$q' in boolean mode))":'').' )';
	$w = count($w)?'where '.implode(' and ',$w):'';
	$pgn = pgnl($x->sql_count="$sql $w",$_qt,$_pg);
	if($pgn->e)$pg = $pgn->c;

	/*$sql .= ' left join cat c on t.idc=c.id';
	...
	if($st=$b->query($sql)){
		...
		while($rs=$st->fetchObject()){
			...
			$rs->t .= '';
			$rs->q += 0;
			$rf = $b->query("select * from fotos where idp={$rs->id} and tipo='team'")->fetchObject();
			$rpr = $b->query("select * from fotos where idp={$rs->id} and tipo='team' and principal")->fetchObject();
			$img = 'thumb/'.($rpr->it?$rpr->it:($rf->it?$rf->it:'default.jpg'));
			$rs->i = $img&&file_exists("upload/team/$img")?$img:'';
