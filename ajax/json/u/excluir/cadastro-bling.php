...
if(true){
	...
	if($apiBling->strApiKey){
		$n = 'Rodrigo Zica de Ananias';
		$tp = 1;
		$cpf_cnpj = '071.152.900-08';
		$ie_rg = '41.119.500-1';
		$rua = 'Winston Churchil';
		$num = 868;
		$bairro = 'Bela Vista';
		$cep = '13321370';
		$city = 'Salto';
		$uf = 'SP';
		$t1 = '(11) 99502-3611';
		$email = 'rodrigo@alquati.com.br';
		$xmlData = array();
		$xmlData['codigo'] = 1;
		$xmlData['nome'] = limitaCaracteres($n,120);//STRING(120)
		$xmlData['tipoPessoa'] = $tp==1?'F':'J';
		$xmlData['contribuinte'] = 9;
		$xmlData['cpf_cnpj'] = $cpf_cnpj;
		$xmlData['ie_rg'] = $ie_rg;//999/9999999
		$xmlData['endereco'] = $rua;
		$xmlData['numero'] = $num;
		$xmlData['complemento'] = $comp;
		$xmlData['bairro'] = $bairro;
		$xmlData['cep'] = $cep;
		$xmlData['cidade'] = $city;
		$xmlData['uf'] = $uf;
		$xmlData['fone'] = $t1;
		$xmlData['celular'] = $t1;
		$xmlData['email'] = $email;
		//$x->bling = $apiBling->postClient($xmlData);
		$x->bling = $apiBling->updateClient($xmlData,1);
	}
	return false;
	...
	//INÍCIO BLING
	//BLING
	$n = 'Rodrigo de Ananias';
	$tp = 1;
	$cpf_cnpj = '369.310.388-77';
	$ie_rg = '41.119.500-1';
	$rua = 'Winston Churchil';
	$num = 868;
	$bairro = 'Bela Vista';
	...
	if($apiBling->strApiKey){
		$xmlData['codigo'] = $id;
		$xmlData['nome'] = limitaCaracteres($n,120);//STRING(120)
		$xmlData['fantasia'] = $r;//STRING
		$xmlData['tipoPessoa'] = $tp==1?'F':'J';//STRING
		$xmlData['contribuinte'] = 9;//1 - Contribuinte do ICMS, 2 - Contribuinte isento do ICMS ou 9 - Não contribuinte
		$xmlData['cpf_cnpj'] = $cpf_cnpj;
		$xmlData['ie_rg'] = $ie_rg;//999/9999999
		$xmlData['endereco'] = $rua;
		$xmlData['numero'] = $num;
...
//$request->success
	...
	elseif(!$email){
		...
		$x->f = 'email';
		if($pg!='checkout')$x->resetCaptcha = 1;//TIRAR COMENTÁRIO DEPOIS
...
/*elseif($tp==1&&!$rg){
	...
	elseif($tp==1&&!$dn){
		...
		$x->f = 'termos';
		if($pg!='checkout')$x->resetCaptcha = 1;
        ...
        <tr>
          <td>
            <table class="containerBox" border="0" align="center" cellpadding="0" cellspacing="0" width="600"  style="border-collapse:collapse;mso-table-lspace:0pt;mso-table-rspace:0pt;">
              ...
              <tr>
                <td align="center"><table class="column" border="0" align="right" cellpadding="0" cellspacing="0" width="250" style="border-collapse:collapse;mso-table-lspace:0pt;mso-table-rspace:0pt;">
                    <tr>
                      <td class="textColor" align="right" valign="top" style="color:#ffffff;font-family:Helvetica,Arial,sans-serif;font-size:14px;line-height:50px;font-weight:300;"><span style="background-color:#211d1e;padding-top:5px;padding-bottom:5px;padding-left:8px;padding-right:8px;border-radius:3px;">'.$telefone.'</span></td>
                    </tr>
                    <tr>
                      <td height="10"></td>
                    </tr>
