...
//INÍCIO REDE PAY
			require_once "erede/Classloader.php";
			$apiKey = '736d2800-dbae-4c88-962f-ab84e202f0a9';
			$tokenNip = '26c844d2-d643-4613-b0f0-fd061bd63585';
			$publicToken = '4528b6ec-12c7-42a8-8f57-3eda54823108';

			try{
				$acquirer = new Acquirer("123", "abc123", EnvironmentType::Production);
				
				$request = new TransactionRequest();
				$request->setCapture("false");
				$request->setAmount(1000);
				$request->setReference("123456X");
				$request->setCardNumber("123456");
				$request->setSecurityCode("123");
				$request->setExpirationMonth(6);
				$request->setExpirationYear(19);
				...
				/* Items */
				$item = new RedePay\Client\Model\SellerItem();
				...
				/* Customer */
				$cellphone = new RedePay\Client\Model\SellerPhone();
				...
				$b->exec("insert into pedido set forma='redepay',dc=now(),idu={$s->idc},tf=$tipo,i='{$cart_ax->qp}',q='{$cart_ax->qtd}',v='{$rc->t}',f='{$x->frete->val}',t='$total',n='$address_nome',rua='$address_rua',num='$address_num',comp='$address_comp',bairro='$address_bairro',cep='$address_cep',city='$address_city',uf='$address_uf',t1='$t1',idc=$idc,w='{$rc->w}'");
				$id = $b->lastInsertId();

				/* Urls */
				/* define a URL para qual o comprador será direcionado após concluir a compra */
				$redirect = new RedePay\Client\Model\SellerUrl();
				...
				$cancel = new RedePay\Client\Model\SellerUrl();
				...
				$notification = new RedePay\Client\Model\SellerUrl();
				...
				$orderNotification = new RedePay\Client\Model\SellerUrl();
				...
				$cliente = new RedePay\Client\ObjectSerializer();
				$response = $api->createOrder("v1",$apiKey,$order);
				//$x->l = $newOrder;
				$x->retorno = $response;
			}catch(RedePay\Api\ApiException $e){
				$x->caughtException = $e->getMessage();
				$x->httpResponseHeaders = $e->getResponseHeaders();
				$x->httpResponseBody = $e->getResponseBody();
				$x->httpStatusCode = $e->getCode();
				//$x->m = $e->getMessage();
				//die($e->getMessage());
			}
//FIM REDE PAY
