...
$rpc = $b->query("select p.n,c.email from pedido p inner join cliente c on p.idu=c.id where p.id=$id limit 1")->fetchObject();
$address_nome = $rpc->n;
$email = $rpc->email;

$send->subject = "Alteração do Pedido n° $id";
$send->html = '
									...
									<td valign="top"><table border="0" cellspacing="0" cellpadding="0" align="center" width="760">
											...
											<tr>
												<td style="color:#1b1b1b;"> Olá '.strh($address_nome).'!<br>
													<br>
													O seu pedido n°: <a href="'.$site.'pedido/'.$id.'"><strong>'.$id.'</strong></a>.<br><br>
													Foi alterado: '.strh($status).'<br><br>
