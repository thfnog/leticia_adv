...
class BlingSDK{
	 ...
	 * @name postOrder
	 * @access public
	 * @internal Insere um cliente/contato no ERP Bling
	 * @author Rodrigo de Ananias
	 * @param array $arrayPreData
	 * @return string | json
	 */

	public function postClient($arrayPreData,$strClientCode = NULL){
		// DEFINE O CONTEXTO DO ENVIO
		$strContext = 'contato';
		// LIMPA CARACTERES DESNECESSÁRIOS NA STRING CNPJ
		if(isset($arrayPreData['cnpj']))$arrayPreData['cnpj'] = str_replace(array('.','-','/'), '', $arrayPreData['cnpj']);
		// LIMPA CARACTERES DESNECESSÁRIOS NA STRING PARA INSCRIÇÃO ESTADUAL
		if(isset($arrayPreData['ie']))$arrayPreData['ie'] = str_replace(array('.','-','/'), '', $arrayPreData['ie']);
		// LIMPA CARACTERES DESNECESSÁRIOS NA STRING RG
		if(isset($arrayPreData['rg']))$arrayPreData['rg'] = str_replace(array('.','-','/'), '', $arrayPreData['rg']);
		// LIMPA CARACTERES DESNECESSÁRIOS NA STRING CEP
		if(isset($arrayPreData['cep']) && !empty($arrayPreData['cep']))$arrayPreData['cep'] = $this->mask($arrayPreData['cep'], '##.###-###');
		// GERA A ARRAY PADRÃO PARA API 2 BLING
	    $arrayData = array("apikey"=>$this->strApiKey, "xml" => rawurlencode($this->buildXml($arrayPreData, $strContext)));
	    // EXECUTA O ENVIO DE DADOS PARA O BLING
	    return $this->sendDataToBling($strContext, 'post', $arrayData, NULL, $strClientCode);
	}
	...
	public function updateClient($arrayPreData, $strClientCode){
		// DEFINE O CONTEXTO DO ENVIO
		$strContext = 'contato';
		// LIMPA CARACTERES DESNECESSÁRIOS NA STRING CNPJ
		if(isset($arrayPreData['cnpj']))$arrayPreData['cnpj'] = str_replace(array('.','-','/'), '', $arrayPreData['cnpj']);
	    ...
	    $arrayData = array("apikey"=>$this->strApiKey, "xml" => rawurlencode($this->buildXml($arrayPreData, $strContext)));
	    // EXECUTA O ENVIO DE DADOS PARA O BLING
	    return $this->sendDataToBling($strContext, 'put', $arrayData, NULL, $strClientCode);
	}
	...
	private function sendDataToBling($strContext, $strAction, $arrayData = NULL, $strResponseFormat = NULL, $strItemCode = NULL){
		...
		$curl_handle = curl_init();
			...
			// SE A REQUISIÇÃO TRATAR-SE DE UM PUT PREPARA AS OPÇÕES DA URL
				...
				// SE O PARÂMETRO $arrayData FOR INFORMADO SIGINIFICA QUE HÁ UM CÓDIGO DE PRODUTO
				// E A URL PARA O POST DEVE INCLUIR O CÓDIGO DO MESMO
				if($strItemCode)$strOptions = '/'.$strItemCode;
			}
