...
class cart{
	public function insert($idp=0,$qt,$idc=0,$prefix='',$cookie=''){
		...
		$rq = $b->query("select peso w,v,c cod,estoque from produto where id=$idp limit 1")->fetchObject();
		$preco_prod = $rq->v;
		...
		elseif($qt>$rq->estoque)$x->shopping->m = 'No momento temos apenas '.$rq->estoque.' unidade'.($rq->estoque>1?'s':'').' em nosso estoque!';
			...
			$rc = $b->query("select *,rua,cep,city,num,bairro,uf,comp from cliente where id=$idc")->fetchObject();
			if(!$sp->rowCount()){
				...
				else $b->exec("insert into $tableCart set dc=now(),idu=$idc,q=1,n='$cli_n',rua='$cli_rua',num='$cli_num',comp='$cli_comp',bairro='$cli_bairro',cep='$cli_cep',city='$cli_city',uf='$cli_uf',t1='$cli_t1'");
				$idc = $b->lastInsertId();
			}
			else{
				$rp = $sp->fetchObject();
			...
			if($idp){
				...
				elseif($qt){
					...
					//CUPOM
					$rcupom = $b->query("select id,idd from cart where id=$idc limit 1")->fetchObject();
					$id_cupom = $rcupom->idd;
					if($id_cupom){
						$ru = $b->query("select * from cupom where id=$id_cupom limit 1")->fetchObject();
						if($ru->tipo_cupom=='produto'){
							$cupom = $b->query("select sum(t) t from (select a.t t from cart_ax a left join cart c on a.idc=c.id left join cupom_ax u on a.idp=u.idp where a.idc='{$rcupom->id}' and u.idc='{$ru->id}' group by a.idp) t1")->fetchObject();
						}elseif($ru->tipo_cupom=='produtos'){
							$cupom = $b->query("select sum(t) t from(select a.t t from cart_ax a left join cart c on a.idc=c.id left join cupom u on c.idd=u.id where a.idc='{$rcupom->id}' group by a.idp) t1")->fetchObject();
						}elseif($ru->tipo_cupom=='categoria'){
							$cupom = $b->query("select sum(t) t from (select a.t t from cart_ax a left join cart c on a.idc=c.id left join produto p on a.idp=p.id left join cat ca on p.idc=ca.id or p.idc2=ca.id left join cupom_ax u on ca.id=u.ida where a.idc='{$rcupom->id}' and u.idc='{$ru->id}' group by a.idp) t1")->fetchObject();
						}
					...
					if($prefix)$b->exec($x->shopping->ups="update $tableCart set da=now(),v='{$rt->tot}',t='$vtot',i='$q',q='{$rt->qtd}',w='{$rt->peso}' where id=$idc");
					else $b->exec($x->shopping->ups="update $tableCart set da=now(),v='{$rt->tot}',t='$vtot',vd='$des',i='$q',q='{$rt->qtd}',w='{$rt->peso}' where id=$idc");
	...
	public function update($id=0,$qt=0,$idc=0,$prefix=''){
		...
		//CUPOM
		if(!$prefix){
			$rc = $b->query("select id,idd from cart where id=$idc limit 1")->fetchObject();
			$id_cupom = $rc->idd;
			if($id_cupom){
				$ru = $b->query("select * from cupom where id=$id_cupom limit 1")->fetchObject();
				if($ru->tipo_cupom=='produto'){
	...
	public function insertLogin($cookie,$id_cliente){
		...
		if($sct->rowCount()){
			...
			if(!$sc->rowCount()){
				...
				$id = $b->lastInsertId();
				while($rat=$sat->fetchObject())$b->query("insert into cart_ax (idc,idp,q,v,t) values('$id','{$rat->idp}','{$rat->q}','{$rat->v}','{$rat->t}')");
