<style>
.main-variacoes{position:relative;margin-bottom:20px}
.main-variacoes .remove-variacoes{position:absolute;right:-15px;cursor:pointer;top:7px;font-size:20px;color:#fd0d13;}
</style>
<div class="col-md-12">
...
?>
					<li role="presentation"><a href="#tab3" data-toggle="tab"><i class="fa icon-size-actual m-r-xs"></i>Variações / Preço</a></li>
...
?>
							...
							<a href="admin/produto-foto/<?=$s->id?>" class="btn btn-danger">REMOVER FOTOS</a>
<?
...
?>
						<div class="tab-pane active fade in" id="tab3">
							...
							<div class="form-group col-xs-12">
								<a class="btn btn-primary btn-addon m-b-sm btn-lg" id="add-variacoes"><i class="fa fa-plus"></i> ADICIONAR VARIAÇÕES</a>
								<div class="clearfix"></div>
								<h4>* Não é possível adicionar a mesma combinação de peso e sabor, mais de uma vez no mesmo produto.</h4>
							</div>
							<div class="clearfix"></div>
<?
$estoques = $b->query("select * from produto_estoque where id_produto='{$s->id}'");
if($s->id&&$estoques->rowCount()){
	while($restoque=$estoques->fetchObject()){
		$rpeso = $b->query("select n from peso where id='{$restoque->id_peso}'")->fetchObject();
		if($restoque->id_sabor)$rsabor = $b->query("select n from sabor where id='{$restoque->id_sabor}'")->fetchObject();
?>
							<div class="main-variacoes">
								<div class="col-xs-12 col-sm-3">
									<select class="js-pesos form-control" name="peso[]"><option value="<?=$restoque->id_peso?>" selected><?=$rpeso->n?></option></select>
								</div>
								<div class="col-xs-12 col-sm-3">
									<select class="js-sabores form-control" name="sabor[]">
<?
if($rsabor){
?>
										<option value="<?=$restoque->id_sabor?>" selected><?=$rsabor->n?></option>
<?
}
?>
									</select>
								...
								<div class="col-xs-12 col-sm-3">
									<input type="number" min="0.01" step="0.10" class="form-control valid" name="adicional[]" placeholder="Valor do adicional da variação" value="<?=$restoque->adicional?>">
								</div>
								<div class="col-xs-12 col-sm-3">
									<input type="number" min="0" step="1" class="form-control valid" name="estoque[]" placeholder="Estoque da variação"  value="<?=$restoque->estoque?>">
								</div>
								<a class="remove-variacoes" title="REMOVER VARIAÇÃO"><span class="icon icon-trash"></span></a>
								<div class="clearfix"></div>
							</div>
<?
	}
?>
							<div class="mais-variacoes"></div>
<?
}else{
?>
							<div class="mais-variacoes"></div>
...
jQuery(function($){
	sdataID = 0;
	$(document).on('click','#add-variacoes',function(){
		sdataID++;
		$('.mais-variacoes').append(
		'<div class="main-variacoes">'+
			'<div class="col-xs-12 col-sm-3"><select class="js-pesos-'+sdataID+' form-control" name="peso[]"></select></div>'+
			'<div class="col-xs-12 col-sm-3"><select class="js-sabores-'+sdataID+' form-control" name="sabor[]"></select></div>'+
			'<div class="col-xs-12 col-sm-3"><input type="number" min="0.01" step="0.10" class="form-control" name="adicional[]" placeholder="Valor do adicional da variação"/></div>'+
			'<div class="col-xs-12 col-sm-3"><input type="number" min="0" step="1" class="form-control" name="estoque[]" placeholder="Estoque da variação"/></div>'+
			'<a class="remove-variacoes" title="REMOVER VARIAÇÃO"><span class="icon icon-trash"></span></a>'+
			'<div class="clearfix"></div>'+
		'</div>');

		$('.js-sabores-'+sdataID).select2({
			placeholder: 'Buscar Sabores',
			ajax: {
				url:'ajax/g/sabores.json',
				dataType:'json',
	...
	$(document).on("click",".remove-variacoes",function(e){
        e.preventDefault();
		$(this).parent('.main-variacoes').remove();
    });
});
</script>
<?
