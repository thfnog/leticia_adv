<div class="col-md-12">
	<div class="panel panel-white">
		<div class="panel-heading clearfix">
			<h4 class="panel-title"><?=$s->id?'Alterar':'Novo'?> Serviço</h4>
		</div>
...
<?
$sfotos = $b->query("select * from fotos where idp='{$s->id}' and tipo='servico'");
if($s->id){
?>
...
?>
					<?php /*?><li role="presentation"><a href="#tab3" data-toggle="tab"><i class="fa fa-cog m-r-xs"></i>Portfolio</a></li><?php */?>
...
?>
							<div class="form-group col-xs-12 col-lg-3">
								<label for="principal-<?=$rfotos->id?>" class="text-center">
									<img src="upload/servicos/thumb/<?=$rfotos->iti?>" class="view-imgs"/>
...
?>
							...
							<a href="admin/servico-foto/<?=$s->id?>" class="btn btn-danger">ALTERAR FOTOS</a>
<?
	}else echo '<a href="admin/servico-foto/'.$s->id.'" class="btn btn-success">ADICIONAR FOTOS</a>';
?>
						</div>
<?
}
?>
						<?php /*?><div class="tab-pane fade" id="tab3">
							<div class="form-group col-md-12">
								<label for="idp">Portfolios Relacionado</label>
								<select class="js-portfolios form-control" id="idp" multiple="multiple" tabindex="-1" style="width:100%" name="idp[]">
<?
if($s->id){
	$st = $b->query("select p.id,p.h1 from portfolio p left join post_relacionado r on p.id=r.id_portfolio where r.id_blog='{$s->id}' order by p.t");
	while($rt=$st->fetchObject()){
?>
									<option value="<?=$rt->id?>" selected="selected"><?=$rt->h1?></option>
<?
	}
}
?>
...
<?
$sp = $b->query("select id,n from tag where idp={$s->id} and tipo='servico' order by n");
$count = $sp->rowCount();
$pj = 0;
?>
