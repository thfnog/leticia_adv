(function($) {
	var _logs = [],
			...
			i.upfile(function(e, d) {
				...
				$.post('ajax/' + (_.u || '') + (_.a || '') + '.json', F.serialize(), function(x) {
					...
					if (x.noi && _.i) $.each(x.noi, function(k, v) {
						if (v) img(_, _.i[k], k), text(_, _.i[k], k), video(_, _.i[k], k);
					});
					//if(x.m)alert(x.m);
					if(x.m){
		...
		video = function(_, video, e) {
			$('#video-view' + (e ? '-' + e : '')).css('display', video ? '' : 'none').find('source').attr('src', video ? video : '');
		},
		_add = function(_, e, c) {
			e = $(e).hide();
		...
		_video = function(_, e) {
			_add(_, $('#video-view' + (e ? '-' + e : '')), '<label><div class="video"><video controls="" name="media"><source></video></div></label>');
		},
