...
(function($){
	...
	stt = function(_,e,id,b,pg){
		...
		e = $(e);
		s = e.hasClass(i[1])?1:0;
		if(l[id]||!confirm((s==0?'Você deseja ativar ':'Você deseja desativar ')+_.mt+'?'))return;
		e.attr('class','ico load');
		l[id] = 1;
		//$.post('ajax/'+u+'/'+a+'.json',{id:id,s:s?0:1,pg:pg},function(x){//ANTIGO COM VÁRIAS PÁGINAS
		$.post('ajax/status.json',{id:id,s:s?0:1,pg:pg},function(x){//NOVO SÓ PÁGINA STATUS
			//if(x.m)alert(x.m);
			if(x.m){
				$('#sys-notification').show();
				var answer = x.ok==1?'success':'danger';
	...
	copy = function(_,e,id,b){
		...
		e = $(e);
		s = e.hasClass(i[1])?1:0;
		if(l[id]||!confirm('Você deseja copiar '+_.mt+'?'))return;
		l[id] = 1;
		$.post('ajax/c/'+_.a+'.json',{id:id},function(x){
			if(x.m){
				$('#sys-notification').show();
				var answer = x.ok==1?'success':'danger';
				var icon = x.ok==1?'check':'question';
	...
	savePos = function(_,e,s,l,m,u){
		...
		_._.e.find('.tab-lista .lrow').each(function(i){
			d['o'+$(this).attr('_id')] = i;
			mhome['home'+$(this).attr('_id')] = $('.home-'+$(this).attr('_id')).is(':checked')?'S':'N';
		});
		var ida = $('.tab-lista .lrow').attr('_ida')?$('.tab-lista .lrow').attr('_ida'):0;
		var idc = $('.tab-lista .lrow').attr('_idc')?$('.tab-lista .lrow').attr('_idc'):0;
		var idf = $('.tab-lista .lrow').attr('_idf')?$('.tab-lista .lrow').attr('_idf'):0;
		var ids = $('.tab-lista .lrow').attr('_ids')?$('.tab-lista .lrow').attr('_ids'):0;
		var idt = $('.tab-lista .lrow').attr('_idt')?$('.tab-lista .lrow').attr('_idt'):0;
		$.post('ajax/'+u+'/'+_.a+'.json',{d:d,ida:ida,idc:idc,idf:idf,ids:ids,idt:idt,mhome:mhome},function(x){
			//if(x.m)alert(x.m);
			if(x.m){
				$('#sys-notification').show();
				var answer = x.ok==1?'success':'danger';
...
//	ob:order-bottom,
				...
				if($.isArray(v.bt))$.each(v.bt,function(j,w){
					...
					if($.type(w)==='object'){
						if(w.tt===0)_w = {m:'s',ld:null,u:null,a:null,i:['iblock','iunblock'],t:['Ativar','Desativar'],ok:null,f:function(e,c,r,t,b){stt(c,e,r.id,b,r.pg);}};
						...
						if(w.tt===5)_w = {m:'s',ld:null,u:null,a:null,i:['copiar'],t:['Copiar'],ok:null,f:function(e,c,r,t,b){copy(c,e,r.id,b,r.pg);}};
	...
	set = function(_){
		...
		if($.isArray(_.rs))$.each(_.rs,function(i,v){
			qodd++;
			var r = $('<tr class="lrow'+(_.img?' img':'')+(qodd%2==0?' even':' odd')+'" _id="'+v.id+'" _ida="'+v.ida+'" _idc="'+v.idc+'" _idf="'+v.idf+'" _ids="'+v.ids+'" _idt="'+v.idt+'"></tr>');
			$.each(_.th,function(j,w){
				var c = $('<td class="cell"></td>'),
				...
				if(w.t===0){
					...
					var a = w.a||'',b = w.b||'',C = v[w.c]||'',img = a+C+b,il = w.il,auto = w.auto,bg = w.bg;
					img = C?img:w.d?a+w.d+b:'';
					if(!C)il = w.dil,auto = w.dauto,bg = w.dbg;
					img = $('<img/>').attr('src',img);
					if(w.iw!=null)img.css('width',$.isNumeric(w.iw)?w.iw+'px':w.iw);
					if(w.ih!=null)img.css('height',$.isNumeric(w.ih)?w.ih+'px':w.ih);
					if(w.im!=null)img.css('margin',$.isNumeric(w.im)?w.im+'px':w.im);
					if(w.imt!=null)img.css('margin-top',$.isNumeric(w.imt)?w.imt+'px':w.imt);
					if(w.iml!=null)img.css('margin-left',$.isNumeric(w.iml)?w.iml+'px':w.iml);
					if(w.l===1)c.addClass('iline');
					else if(w.l===2){
						c.addClass('box-img');
						img = $('<span><span></span></span>').addClass('box-img-center').append(img);
						if(il)img.addClass('load');
						if(auto)img.addClass('auto');
						if(bg===1)img.addClass('w');
						if(bg===2)img.addClass('b');
					}
					c.append(img);
				}else if(w.t===3){
					var a = w.a||'',b = w.b||'',C = v[w.c]||'',img = a+C+b,il = w.il,auto = w.auto,bg = w.bg;
					cor = C?img:w.d?a+w.d+b:'';
					if(!C)il = w.dil,auto = w.dauto,bg = w.dbg;
					cor = $('<div style="width:103px;height:84px;background:'+cor+'"/>');
					if(w.l===1)c.addClass('iline');
					else if(w.l===2){
						c.addClass('box-img');
						img = $('<span><span></span></span>').addClass('box-img-center').append(img);
					...
					var a = w.a||'',b = w.b||'',C = v[w.c]||'',img = a+C+b,il = w.il,auto = w.auto,bg = w.bg;
					sel = C?img:w.d?a+w.d+b:'';
					if(!C)il = w.dil,auto = w.dauto,bg = w.dbg;
					//sel = $('<select class="form-control" _home-'+v.id+'="'+v.id+'"><option value="S">SIM</option><option value="N">NÃO</option></select>');
					sel = $('<input type="checkbox" class="form-control home-'+v.id+'" checked value="1" style="width:20px"/>');
					if(w.l===1)c.addClass('iline');
					c.append(sel);
				}
