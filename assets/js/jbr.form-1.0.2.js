/* Edição: 28/04/2016 17:39 / Modificado init linha 295 */(function($) {
	var _logs = [],
		l = function(_, a) {
		...
		up = function(_) {
			...
			i.upfile(function(e, d) {
				...
				$.post('ajax/' + (_.u || '') + (_.a || '') + '.json', F.serialize(), function(x) {
					...
					//if(x.m)alert(x.m);
					/*if(x.indique && x.m){
						$('#sys-notification2').show();
						var answer = x.ok==1?'success':'danger';
						var icon = x.ok==1?'check':'question';
						$('#notification2').html('<div class="alert alert-'+answer+'"><i class="fa fa-'+icon+'-circle"></i> '+x.m+'<button type="button" class="close" data-dismiss="alert">×</button></div>').show();
					...
					if(x.news){
						$('#sys-notification-news').show();
						var answer = x.ok==1?'success':'danger';
						var icon = x.ok==1?'check':'question';
						$('#news-notification').html('<div class="alert alert-'+answer+'"><i class="fa fa-'+icon+'-circle"></i> '+x.news+'<button type="button" class="close" data-dismiss="alert">×</button></div>').show();
						setTimeout(function(){$('#news-notification').hide();},5000);
					}
					if(x.e)$.each(x.e,function(k,v){
						var placeholder = $("#"+k).attr('placeholder');
						$("#"+k).attr('data-placeholder',placeholder);
						$("#"+k).addClass('haserror').css('border-color','#f00').attr('placeholder',v);
						return hasError(k);
					});
					if(x.f){
						$("#"+x.f).focus().addClass('haserror').css('border-color','#f00').scrollTop();
						return hasError(x.f);
					}
